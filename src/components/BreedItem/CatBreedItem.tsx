...
const CatBreedItem = ({ breedId }: { breedId: string }) => {
  const styles = useThemedStyles(createStyles);

  const breed = useStore((state) => state.catBreeds[breedId]);
  if (!breed?.name) return null;
  const isSelected = useStore(
    (state) => state.selectedCatBreeds[breedId] ?? false,
  );
  const toggleBreed = useStore((state) => state.toggleCatBreed);
